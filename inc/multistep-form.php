...
function custom_multistep_form() {
    ...
    if (isset($_GET['bd']) && $_GET['bd'] == 'newform' && !isset($_GET['new_id'])) {
        ...
        $new_form_id = !empty($saved_forms) ? max($saved_forms) + 1 : 1;
    ...
    $attachment = get_user_meta($user_id, "multi_step_attachment_{$form_id}", true);
    ...
    <form id="multiStepForm" method="post" action="">
        ...
        <div class="step step-2" style="display: none;">
            <div class="card mb-4">
                <div class="card-body">
                    <h4 class="card-title">Upload Attachments</h4>               
                    <div class="form-row img-wrapper">
                        <div class="form-group col-md-12 mb-3">
                            <div class="upload-btn-wrapper">
                                <button class="btn btn-theme mt-2 mt-xl-0" type="button">Upload Attachments</button>
                                <input type="file" id="attachment" name="nidd" accept=".jpg,.jpeg,.png,.pdf">
                            </div>
                        </div>
                    </div>
                    <?php if (!empty($attachment)) : ?>
                        <div class="form-row">
                            <div class="form-group col-md-12 attachment-box" style="display: block;">
                                <p><strong>Uploaded File:</strong></p>
                                <?php 
                                $file_extension = pathinfo($attachment, PATHINFO_EXTENSION);
                                if (in_array($file_extension, ['jpg', 'jpeg', 'png'])) : ?>
                                    <img src="<?php echo esc_url($attachment); ?>" alt="Uploaded Image" style="max-width: 100px; display: block;">
                                <?php else : ?>
                                    <a href="<?php echo esc_url($attachment); ?>" target="_blank">View Uploaded File</a>
                                <?php endif; ?>
                            </div>                       
                        </div>
                    <?php endif; ?>
                    <div class="form-row">
                        <div class="form-group col-md-12 attachment-box" style="display: block;"></div>                       
                    </div>
                </div>
            </div>
            <h3>Identity</h3>
